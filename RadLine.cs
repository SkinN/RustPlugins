...
{
    [Info("RAD-Line", "SkinN", "3.0.0", ResourceId = 914)]
    [Description("Turns off radiation for X minutes")]
    ...
    class RadLine : RustPlugin
    {
        ...
        void Init()
        {
            // Check for Zones Manager
            ZoneManager = plugins.Find("ZoneManager");

            if (ZoneManager == null)
            {
                PrintWarning("Zones Manager not detected!");
                PrintWarning("Since Rust developers have disabled Radiation in the game due to Rust development");
                PrintWarning("it's higly recommended to install Zones Manager plugin and manually add radiaton zones");
                PrintWarning("throghout the map for RAD-Line to work properly, this is a temporary work-around");
            }
            else
            {
                PrintWarning("Zones Manager detected!");
                PrintWarning("It's higly recommended to use Zones Manager plugin and manually add radiaton zones");
                PrintWarning("throghout the map for RAD-Line to work properly, this is a temporary work-around");
            }
